...
def test_join_movie_data(
    movies_ht: hl.Table, ratings_ht: hl.Table, users_ht: hl.Table
) -> None:
    """
    Test function to verify the correctness of the join_movie_data function.
    It checks if the resulting table has the correct fields, and whether these fields contain expected data from the joined tables.
    """
    # Perform the join operation using the defined function
    joined_table = join_movie_data(movies_ht, ratings_ht, users_ht)
    ...
    result_fields = set(joined_table.row_value.keys())
    ...
    sample_user_id = 196
    user_data = joined_table.filter(joined_table.user_id == sample_user_id).collect()
